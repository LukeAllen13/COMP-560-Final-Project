...
function App() {
  ...
  const [refereeCommentary, setRefereeCommentary] = useState<string | null>(
    null
  );
  ...
  const handleSquareClick = async (x: number, y: number) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Update evaluation score from backend
        fetchEvaluation().then(setEvaluationScore);
        ...
        if (newMoveCount % 5 === 0) {
          ...
          try {
            ...
            setRefereeCommentary(
              "Unable to fetch commentary at this time."
            );
          } finally {
  ...
  return (
    <div className="app">
      ...
      {/* Evaluation bar using evaluationScore from backend */}
      ...
      {showCommentary && (
        <div className="commentary-bubble">
          <div className="commentary-header">
            <span className="commentary-title">Referee Commentary</span>
            <button
